...
Options.init('profileSchema', {
  name: {
    type: String,
    label: scorpius.helpers.getTranslation('accounts.schema.profile.name')
  }
});
...
Tracker.autorun(function () {
  scorpius.accounts.profileSchema = new SimpleSchema({
    profile: {
...
Options.listen('forbidClientAccountCreation', function(value) {
  AccountsTemplates.configure({
    forbidClientAccountCreation: (!!scorpius.adminExists) && (!!value),
  });
});
...
UsersEmailsSchema = new SimpleSchema({
  emails: {
    type: [Object],
    optional: true,
    label: scorpius.helpers.getTranslation('accounts.schema.emails.title')
  },
  'emails.$.address': {
    type: String,
    regEx: SimpleSchema.RegEx.Email,
    label: scorpius.helpers.getTranslation('accounts.schema.emails.address')
  },
  'emails.$.verified': {
    type: Boolean,
    label: scorpius.helpers.getTranslation('accounts.schema.emails.verified')
  }
});
...
UsersPasswordSchema = new SimpleSchema({
  password: {
    type: String,
    label: scorpius.helpers.getTranslation('accounts.schema.password.new'),
    min: 8,
    autoform: {
      type: 'password'
    }
  ...
  confirm: {
    type: String,
    label: scorpius.helpers.getTranslation('accounts.schema.password.confirm'),
    min: 8,
    autoform: {
      type: 'password'
    },
